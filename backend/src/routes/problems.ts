...
const problemInput = z.object({
  ...
  language: z.literal("cpp17").optional(),
  starterCode: z.string().max(65536).optional(),
...
problemsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const body = req.body as z.infer<typeof problemInput>;
    await getAssignmentOr404(body.assignmentId);
    const p = await Problem.create(body);
    res.status(201).json({ problem: toPublicProblem(p) });
  })
);
...
problemsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    await p.save();
    res.json({ problem: toPublicProblem(p) });
  })
);
...
problemsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const p = await getProblemOr404(req.params.id);
    const { referenceSolution } = req.body as { referenceSolution: string };
    const { verifyProblemSolution } = await import("../services/verifyService.js");
    const result = await verifyProblemSolution(String(p._id), referenceSolution);
    res.json(result);
  })
);
